...
ui = fluidPage(
  ...
  sliderInput(inputId = "q1", 
              label = "Q1: Rate your frequency of social interaction with others excluding family
              from 0 (no interaction) to 10 (very frequent interaction).", 
              value = 0, min = 0, max = 10),
  
  sliderInput(inputId = "q2", 
              label = "Q2: Rate your physical health from 0 (no symptoms, healthy) to 10 
...
server = function(input, output) {
  output$score = renderText({
    paste(c("Mental Health Score (0 (Poor Mental Health) to 10 (Excellent Mental Health)) ---> ", round((((final.model(input$q1, input$q2, input$q3, input$q4, input$q5,input$q6, input$q7, input$q8, input$q9, input$q10)-0.838710000000001)/19.01088)*10), 3)), collapse = " ")
  })
}
